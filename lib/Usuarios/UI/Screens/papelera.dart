...
class PapeleraMensaje extends StatefulWidget{
  @override
  State createState() {
    return _PapeleraMensaje();
  }
}
class _PapeleraMensaje extends State<PapeleraMensaje>{
  @override
  String _Msjbuscar="";
  String _Seccion='papelera';
  ...
  Widget correo(Mensaje msj){
    return Column(
      children: [
        GestureDetector(
          onTap: (){
            Navigator.push(context, PageTransition(child: Mensajes_buzon_detail(msj: msj,desdeSeccion: msj.SecAnterior,), type: PageTransitionType.rightToLeft));
          },
          child: Container(
                  ...
                  child: Container(
                    decoration: BoxDecoration(
                      ...
                      image: DecorationImage(
                          fit: BoxFit.cover,
                          image: msj.SecAnterior=='entrada'? NetworkImage(msj.fotoremit): NetworkImage(msj.fotodest)
                      ),
                    ),
                  ),
                ...
                Column(
                  ...
                  children: [
                    Container(
                      ...
                      child: Text(msj.SecAnterior=='entrada'?msj.Nremitente:msj.Ndestinatario,
                        style: TextStyle(
                                ...
                                onDismissed: (direction) {
                                    if(direction.toString()=='DismissDirection.endToStart'){
                                      ...
                                      ScaffoldMessenger.of(context)
                                          .showSnackBar(SnackBar(duration: Duration(seconds: 1),
                                                          content: Text('Mensaje eliminado.',textAlign: TextAlign.center,)
                                                          ,backgroundColor: Colors.red,));
                                    }else{
                                      FirestoreAPI(TipMensaje: _Seccion,accion: 'recuperar').eliminaMensaje(lstMensajes[i],_Seccion);
                                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(duration: Duration(seconds: 2),
                                        content: Text('Mensaje recuperado en '+lstMensajes[i].SecAnterior,textAlign: TextAlign.center,)
                                        ,backgroundColor: Color(0Xff2056a1),));
                                    }
                                   },
